...
frappe.ui.form.on("Sales Invoice Item", {
    ...
    item_code(frm, cdt, cdn) {
        set_secondary_fields_si(frm, cdt, cdn);
    },

    qty(frm, cdt, cdn) {
        update_from_primary_si(frm, cdt, cdn);
    },
    ...
    secondary_qty(frm, cdt, cdn) {
        update_from_secondary_si(frm, cdt, cdn);
    },

    secondary_conversion_factor(frm, cdt, cdn) {
        if (updating) return;
        update_on_factor_change_si(frm, cdt, cdn);
    }
});


function set_secondary_fields_si(frm, cdt, cdn) {
...
function update_from_primary_si(frm, cdt, cdn) {
    if (updating) return;
    updating = true;
...
function update_from_secondary_si(frm, cdt, cdn) {
    if (updating) return;
    updating = true;
...
function update_on_factor_change_si(frm, cdt, cdn) {
    if (updating) return;
    updating = true;
